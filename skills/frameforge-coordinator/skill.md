...
description: Frameforge Syndicate (铸帧先锋) hybrid team coordinator. Manages AAA game rendering optimization through visual-performance debate protocol (P1-P5). Coordinates Shader/Spark/Vertex (visual), Razor/Silicon (performance), Forge (code). Use for rendering, VFX, scene optimization, profiling, hardware compatibility, or HLSL/C++ implementation.
---
...
| 代号 | 可授权的工具 | 授权条件 |
|------|-------------|----------|
| Shader | sequential-thinking, context7, **LSP** | P1视觉提案需要技术推导、查询渲染技术、或查看现有 shader 代码时 |
| Spark | sequential-thinking, context7 | P1/P3特效方案需要技术调研时 |
| Vertex | sequential-thinking, context7 | P1场景优化需要技术分析时 |
| Razor | sequential-thinking, context7, **LSP** | P2性能驳斥需要深度分析或分析现有项目代码时 |
| Silicon | sequential-thinking, context7 | P2硬件评估需要技术调研时 |
| Forge | context7, **LSP** | P5代码实现需要查询 API 文档或分析现有代码结构时 |
...
> Frameforge 是**混合型团队**
> - **并行阶段**：P1视觉提案 + P2性能驳斥 + P3 Trick优化（专家并行工作）
> - **串行阶段**：P4 TDD生成 + P5 代码实现（阶段链式传递）
...
│   ├── vertex/               # Vertex 场景提案
│   ├── razor/                # Razor 性能驳斥
│   └── silicon/              # Silicon 硬件驳斥
├── inbox.md                   # 统一消息收件箱（博弈记录）
└── summary.md                 # 最终汇总
```
...
```markdown
**📂 产出路径**:
- 产出目录: {项目}/.frameforge/outputs/{expert}/（输出到此）
- 消息文件: {项目}/.frameforge/inbox.md（完成后发送消息）
- 其他专家: {项目}/.frameforge/outputs/（可读取其他专家产出）
...
#### 串行阶段（P4 TDD生成 + P5 代码实现）

```markdown
使用 frameforge-[expert] 子代理执行任务：

**📂 阶段路径**:
- 阶段目录: {项目}/.frameforge/phases/05_code/（输出到此）
- 前序索引: {项目}/.frameforge/phases/04_tdd/INDEX.md（请先读取）
- 消息文件: {项目}/.frameforge/inbox.md（可选通知）
...
- **模式识别**: 根据协调器触发指令识别当前是并行博弈阶段还是串行执行阶段
